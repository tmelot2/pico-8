...
function deadupdate()
  ...
  delete_cnt=maxplayerh/120
	...
	if #player.h > 0 then
		...
		pauseTimer += 1
		if pauseTimer > deadDuration then
			checkHighScoreInit()
		end
	end
end
...
function deaddraw()
  ...
  SIDE_RATE=0.05
  MAX_RATE = 1500
  rate = MAX_RATE * (pauseTimer/(deadDuration/2))
  rate=1200*rnd()+200
  for i=1,rate do
    x=flr(rnd(128))
    y=flr(rnd(128))
    c=pget(x,y)
    if c>0 then
      if rnd()<DRIP_RATE then
        if y+1<128 and pget(x,y+1)==0 then
          pset(x,y+1,c)
        end
      end
      if rnd()<ERASE_RATE then
        pset(x,y,0)
      end
      if rnd()<SIDE_RATE and x>0 and x<127 then
        pset(x+(rnd()<0.5 and -1 or 1),y,c)
      end
    end
  end
...

